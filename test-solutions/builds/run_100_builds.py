...
def _generate_regulations_md(sol: dict, detected_domains: list) -> str:
    """Generate regulations.md with compliance requirements."""
    lines = [
    ...
    lines.append("|---|---|---|")
    for std in sol["compliance"]:
        lines.append(f"| {std} compliance verified | PENDING | — |")
    lines.append("")
    lines.append("## Audit Trail")
    lines.append("")
    lines.append(f"Generated by SAGE Build Orchestrator at {datetime.now().isoformat()}")
    lines.append(f"All build artifacts subject to HITL approval gate.")
    lines.append("")
    return "\n".join(lines)
...
def build_solution(sol: dict, idx: int, total: int) -> dict:
    ...
    with open(os.path.join(folder, "regulations.md"), "w") as f:
        f.write(_generate_regulations_md(sol, detected_domains))

    print("done")

    # Step 5: Approve plan (auto-approve to trigger scaffold phase)
    print(f"  → POST /build/approve/{run_id[:8]}... ...", end=" ", flush=True)
    approve_result = api_post(f"/build/approve/{run_id}", {
        "approved": True,
        "feedback": "Auto-approved for scale deployment test",
    })
    approve_status = approve_result.get("state", approve_result.get("error", "unknown"))
    print(f"state={approve_status}")

    # Step 6: Get updated status after approval
    final_status = api_get(f"/build/status/{run_id}")
