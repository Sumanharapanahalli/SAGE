...
def _generate_regulations_md(sol: dict, detected_domains: list, plan: list,
                              critic_data: dict | None = None) -> str:
    """Generate regulations.md with compliance requirements, plan evidence, and critic review."""
    lines = [
    ...
    # Section 7: Critic Review (actor-critic assessment)
    lines.append("")
    lines.append("## 7. Critic Review (Actor-Critic Assessment)")
    lines.append("")
    if critic_data:
        scores = critic_data.get("critic_scores", [])
        reports = critic_data.get("critic_reports", [])
        if scores:
            plan_score = next((s for s in scores if s.get("phase") == "plan"), {})
            score_val = plan_score.get("score", "N/A")
            passed = plan_score.get("passed", False)
            iters = plan_score.get("iterations", 1)
            badge = "PASS" if passed else "FAIL"
            lines.append(f"**Plan Score:** {score_val}/100 ({badge}) — {iters} iteration(s)")
            lines.append("")
        if reports:
            plan_report = next((r for r in reports if r.get("phase") == "plan"), {})
            result = plan_report.get("result", {})
            final = result.get("final_review", {})
            if final:
                lines.append(f"**Summary:** {final.get('summary', 'N/A')}")
                lines.append("")

                flaws = final.get("flaws", [])
                if flaws:
                    lines.append("### Flaws Identified")
                    lines.append("")
                ...
                security = final.get("security_risks", [])
                if security:
                    lines.append("### Security Risks")
                    lines.append("")
                    for i, risk in enumerate(security, 1):
                        lines.append(f"{i}. {risk}")
                    lines.append("")
    else:
        lines.append("*No critic review data available.*")
        lines.append("")
...
def build_solution(sol: dict, idx: int, total: int) -> dict:
    ...
    # regulations.md (includes critic review data)
    critic_data = {
        "critic_scores": status.get("critic_scores", []),
        "critic_reports": status.get("critic_reports", []),
    }
...
def regenerate_docs():
    ...
    for sol in SOLUTIONS:
        ...
        with open(os.path.join(folder, "regulations.md"), "w") as f:
            f.write(_generate_regulations_md(sol, detected_domains, plan, critic_data))
        count += 1
        print(f"  Regenerated {sol['id']}/{sol['name']} regulations.md")
