...
class TestE2EScenario_NonDeviceCDS:
    ...
    def test_step1_classify_function(self):
        ...
        assert data["criterion_1"]["passes"] is True, (
            "Criterion 1 should pass — no images/signals"
        )
        assert data["criterion_2"]["passes"] is True, (
            "Criterion 2 should pass — FDA labeling source"
        )
        assert data["criterion_3"]["passes"] is True, (
            "Criterion 3 should pass — physician, recommendation"
        )
        assert data["criterion_4"]["passes"] is True, (
            "Criterion 4 should pass — non-urgent"
        )
    ...
    def test_step1_classify_as_device(self):
        ...
        resp = client.post(
            ...
            json={
                ...
                "urgency": "non_urgent",
                "data_sources": [
                    {"name": "ACR Lung-RADS Guidelines", "type": "clinical_guideline"}
                ],
            },
        )
        assert resp.status_code == 200
    ...
    def test_step3_package_still_generates(self):
        ...
        resp = client.post(
            ...
            json={
                ...
                "urgency": "non_urgent",
                "data_sources": [
                    {"name": "ACR Guidelines", "type": "clinical_guideline"}
                ],
    ...
    def test_time_critical_fails_criterion_4(self):
        ...
        resp = client.post(
            ...
            json={
                ...
                "data_sources": [
                    {"name": "Surviving Sepsis Campaign", "type": "clinical_guideline"}
                ],
            },
        )
        assert resp.status_code == 200
...
class TestE2EUIHealthCheck:
    ...
    def test_all_cds_endpoints_exist(self):
        ...
        endpoints = [
            ...
            "/cds/validate-sources",
            "/cds/classify-output",
            "/cds/transparency-report",
            "/cds/labeling",
            "/cds/bias-warning",
            "/cds/bias-risk",
            "/cds/clinical-limitations",
            "/cds/validation-protocol",
            "/cds/over-reliance",
            "/cds/reevaluate",
            "/cds/patient-population",
            "/cds/compliance-package",
        ]
        for ep in endpoints:
    ...
    def test_openapi_includes_cds_routes(self):
        ...
        assert len(cds_paths) >= 14, (
            f"Expected 14+ CDS paths, got {len(cds_paths)}: {cds_paths}"
        )
