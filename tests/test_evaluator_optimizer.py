...
def _run(opt, ev, **cfg):
    ...
    base = {
        ...
        "evaluator_provider": ev,
        "max_iterations": 4,
        "score_threshold": 8.0,
        "generate_rubric": False,
    }
    base.update(cfg)
    return EvaluatorOptimizerRunner(base).run(
        "improve the thing", context="current artifact"
    )
...
def test_converges_when_evaluator_passes():
    ev = MockEvaluator(
        [
            '{"score": 6, "pass": false, "feedback": "fix the edge case"}',
...
def test_parse_evaluation_distinguishes_genuine_zero_from_parse_failure():
    ...
    good = EvaluatorOptimizerRunner._parse_evaluation(
        '{"score": 0, "pass": false, "feedback": "bad"}'
    )
    ...
    bad = EvaluatorOptimizerRunner._parse_evaluation(
        "the model wrote prose and emitted no JSON"
    )
...
def test_best_candidate_ignores_unparseable_evaluations():
    ...
    ev = MockEvaluator(
        ['{"score": 5, "pass": false, "feedback": "ok"}', "garbage, not json at all"]
    )
    r = _run(MockOptimizer(), ev, max_iterations=3)
    assert r["converged"] is False
    assert r["score"] == 5
...
def test_optimizer_receives_evaluator_feedback():
    ...
    ev = MockEvaluator(
        [
            '{"score": 5, "pass": false, "feedback": "ADDRESS_THIS_POINT"}',
            '{"score": 9, "pass": true, "feedback": ""}',
        ]
    )
    _run(opt, ev)
...
def test_extract_json_tolerates_fences_and_prose():
    ...
    assert (
        _extract_json('Here is my verdict: {"score": 3, "pass": false} done')["score"]
        == 3
    )
...
def test_missing_providers_returns_error():
    r = EvaluatorOptimizerRunner(
        {"optimizer_provider": None, "evaluator_provider": None}
    ).run("x")
...
def test_rubric_is_generated_and_fed_into_evaluation():
    ...
    r = EvaluatorOptimizerRunner(
        {
            "optimizer_provider": MockOptimizer(),
            "evaluator_provider": ev,
...
def test_sandbox_hardening_applied_to_built_optimizer():
    ...
    EvaluatorOptimizerRunner(
        {
            "optimizer": {"provider": "claude-code"},
            "evaluator_provider": MockEvaluator(["{}"]),
        },
...
def test_evaluator_pool_aggregates_via_median_not_mean():
    ...
    r = _run(
        MockOptimizer(),
        None,
        evaluator_pool_providers=providers,
        max_iterations=1,
        score_threshold=8.0,
    )
    assert r["score"] == 8.0
...
def test_evaluator_pool_all_unparseable_flags_evaluator_unparseable():
    ...
    r = _run(
        MockOptimizer(), None, evaluator_pool_providers=providers, max_iterations=1
    )
    assert r["converged"] is False
    assert r.get("evaluator_unparseable") is True
    assert r["score"] == 0.0
...
def test_evaluator_pool_merges_feedback_from_all_providers_that_failed():
    providers = [
        ...
        MockEvaluator(
            ['{"score": 4, "pass": false, "feedback": "add error handling"}']
        ),
    ]
    r = _run(
        MockOptimizer(),
