...
def _validate_record(table: str, record: dict) -> list[_Check]:
    ...
    if table == "proposals":
        ...
        if record.get("message_type"):
            checks.append(
                _enum_check(record, "message_type", ["user", "assistant", "system"])
            )
...
SEED_CHAT_MESSAGES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "metadata": json.dumps(
            {"test_marker": True, "trace_id": "trace-lifecycle-001"}
        ),
    },
    {
...
class ApiTestDataSeeder:
    ...
    def teardown_all(self) -> None:
        ...
        try:
            conn.execute("DELETE FROM chat_messages WHERE user_id LIKE 'test_%'")
            conn.execute("DELETE FROM compliance_audit_log WHERE actor LIKE 'test_%'")
            conn.execute("DELETE FROM proposals WHERE proposed_by LIKE 'test_%'")
            conn.commit()
        finally:
            conn.close()
    ...
    def _seed_audit_events(self) -> None:
        ...
        logger.info(
            "compliance_audit_log: inserted=%d  skipped_validation=%d",
            inserted,
            skipped_validation,
        )
    ...
    def _seed_chat_messages(self) -> None:
        ...
        logger.info(
            "chat_messages: inserted=%d  skipped_validation=%d",
            inserted,
            skipped_validation,
        )
    ...
    def _seed_proposals(self) -> None:
        ...
        logger.info(
            "proposals: inserted=%d  skipped_validation=%d",
            inserted,
            skipped_validation,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--db", default=None, help="Path to SQLite DB (default: .sage/audit_log.db)"
    )
    parser.add_argument(
        "--teardown", action="store_true", help="Remove test rows instead of seeding"
    )
