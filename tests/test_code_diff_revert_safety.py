...
def test_reject_deletes_only_the_agents_new_file(repo):
    ...
    assert (repo / "operator_scratch.txt").read_text(
        encoding="utf-8"
    ) == "MY UNSAVED WORK\n"
...
def test_reject_restores_a_modified_tracked_file_without_touching_others(repo):
    ...
    (repo / "other_tracked.py").write_text(
    ...
    assert (repo / "other_tracked.py").read_text(
        encoding="utf-8"
    ) == "OPERATOR EDIT IN PROGRESS\n"
...
def test_reject_with_no_recorded_files_wipes_nothing(repo):
    ...
    assert (repo / "precious.txt").exists(), (
        "empty payload must NOT trigger a tree wipe"
    )
