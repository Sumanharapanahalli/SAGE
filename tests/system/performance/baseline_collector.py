...
CRITICAL_PATHS: List[Dict[str, Any]] = [
    ...
    },
    {
        "name": "invest_order",
        "method": "POST",
        "path": "/api/v1/investments/orders",
        "samples": 50,
        "requires_auth": True,
    },
    {
...
async def _get_test_transfer(
    client: httpx.AsyncClient, token: str, account_id: str
) -> Optional[str]:
    headers = {"Authorization": f"Bearer {token}"}
    dest = await client.post(
        "/api/v1/accounts", json={"account_type": "savings"}, headers=headers
    )
    ...
    for i in range(path_def["samples"]):
        ...
        if path_def["method"] == "POST":
            if "register" in url:
                ...
                payload = {
                    "email": "preseeded_0001@test-staging.example.com",
                    "password": "PreSeeded@99!",
                }
                ...
                dest_id = (
                    dest_resp.json().get("account_id", "acc_unknown")
                    if dest_resp.status_code == 201
                    else "acc_unknown"
                )
                payload = {
                    "from_account_id": account_id,
...
async def collect_all(target: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    if target:
        ...
        if not paths:
            raise ValueError(
                f"Unknown endpoint: {target}. Valid: {[p['name'] for p in CRITICAL_PATHS]}"
            )
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=30.0) as client:
        ...
        transfer_id = (
            await _get_test_transfer(client, token, account_id)
        ...
        for path_def in paths:
            ...
            result = await measure_path(
                client, path_def, token, account_id, transfer_id
            )
            ...
            print(
                f"    p99={result.get('p99_ms')}ms  SLA={'PASS' if sla_ok else 'FAIL'}"
            )
...
def main():
    ...
    parser.add_argument(
        "--endpoint", help="Single endpoint name to measure", default=None
    )
