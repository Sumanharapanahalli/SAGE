...
def _enum_check(record: dict, field_name: str, allowed: list[str]) -> _Check:
    ...
    return _Check(
        ...
        passed=ok,
        message=f"'{field_name}' must be one of {allowed}, got {val!r}."
        if not ok
        else "ok",
    )
...
def _validate_record(table: str, record: dict) -> list[_Check]:
    ...
    if table == "accounts":
        ...
        checks += _require_fields(
            record, ["sender_id", "receiver_id", "amount", "status"]
        )
        checks += _positive(record, ["amount"])
        checks.append(
            _enum_check(
                record, "status", ["pending", "completed", "failed", "reversed"]
            )
        )
        ...
        checks.append(
            _Check(
                ...
                passed=no_self,
                message=f"sender_id and receiver_id must differ (got {sender})."
                if not no_self
                else "ok",
                severity="critical",
            )
        ...
        checks += _require_fields(
            record, ["card_holder_id", "card_type", "card_expiry_date", "status"]
        )
        ...
        if expiry:
            ...
            checks.append(
                _Check(
                    ...
                    passed=future,
                    message=f"card_expiry_date {expiry} is in the past."
                    if not future
                    else "ok",
                    severity="warning",
                )
        ...
        checks += _require_fields(
            record, ["name", "asset_type", "risk_level", "return_potential"]
        )
        checks.append(
            _enum_check(record, "risk_level", ["low", "medium", "high", "very_high"])
        )
...
def _is_valid(table: str, record: dict, row_label: str) -> bool:
    ...
    blocking = [
        c for c in checks if not c.passed and c.severity in ("error", "critical")
    ]
    if blocking:
        for c in blocking:
            logger.warning(
                "SKIP %s [%s] — %s (%s)", table, row_label, c.message, c.rule
            )
        return False
...
SEED_TRANSFERS_TEMPLATE: list[dict[str, Any]] = [
    {
        ...
        "status": "completed",
        "reference": "TEST-TXN-0001",
        "notes": "Split dinner",
    },
    {
        "_sender": "bob_test",
        "_receiver": "carol_test",
        "amount": Decimal("50.00"),
        "status": "completed",
        "reference": "TEST-TXN-0002",
        "notes": "Rent share",
    },
    {
        "_sender": "alice_test",
        "_receiver": "frank_test",
        "amount": Decimal("200.00"),
        "status": "pending",
        "reference": "TEST-TXN-0003",
        "notes": "Pending payment",
    },
    {
        "_sender": "frank_test",
        "_receiver": "alice_test",
        "amount": Decimal("75.25"),
        "status": "completed",
        "reference": "TEST-TXN-0004",
        "notes": "Refund",
    },
    {
        "_sender": "carol_test",
    ...
    },
    {
        ...
        "status": "reversed",
        "reference": "TEST-TXN-0007",
        "notes": "Duplicate — reversed",
    },
    {
        "_sender": "frank_test",
        "_receiver": "bob_test",
...
SEED_CARDS_TEMPLATE: list[dict[str, Any]] = [
    {
        "_holder": "alice_test",
        "card_type": "debit",
        "card_expiry_date": _EXPIRY_ACTIVE,
        "card_last_four": "1234",
    ...
    },
    {
        "_holder": "carol_test",
        "card_type": "prepaid",
        "card_expiry_date": _EXPIRY_FROZEN,
        "card_last_four": "9012",
    ...
    },
    {
        "_holder": "frank_test",
        "card_type": "debit",
        "card_expiry_date": _EXPIRY_WARNING,
        "card_last_four": "3456",
...
SEED_INVESTMENTS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "asset_type": "bonds",
        "risk_level": "low",
        "return_potential": Decimal("5.00"),
        "min_investment": Decimal("100.00"),
        "is_active": True,
    },
    {
        "name": "Test Equity Index",
        "asset_type": "equities",
        "risk_level": "medium",
    ...
    {
        "name": "Test Growth ETF",
        "asset_type": "equities",
        "risk_level": "high",
        "return_potential": Decimal("14.00"),
        "min_investment": Decimal("500.00"),
        "is_active": True,
    },
    {
        "name": "Test Crypto Basket",
        "asset_type": "crypto",
        "risk_level": "very_high",
        "return_potential": Decimal("35.00"),
...
class TestDataSeeder:
    ...
    def teardown_all(self) -> None:
        ...
        with self._session_factory() as session:
            ...
            session.execute(
                text("DELETE FROM p2p_transfers WHERE reference LIKE 'TEST-TXN-%'")
            )
            session.execute(
                text("DELETE FROM investment_options WHERE name LIKE 'Test %'")
            )
            session.execute(text("DELETE FROM accounts WHERE username LIKE '%_test'"))
            session.commit()
    ...
    def _seed_accounts(self) -> None:
        ...
        with self._session_factory() as session:
            ...
            session.commit()
        self.stats["accounts"] = {
            "attempted": len(SEED_ACCOUNTS),
            "inserted": inserted,
            "skipped_validation": skipped_validation,
        }
        logger.info(
            "accounts: inserted=%d  skipped_validation=%d", inserted, skipped_validation
        )
    ...
    def _resolve_account_ids(self) -> None:
        ...
        with self._session_factory() as session:
            rows = session.execute(
                text(
                    "SELECT account_id, username FROM accounts WHERE username = ANY(:u)"
                ),
                {"u": usernames},
            ).fetchall()
    ...
    def _seed_investments(self) -> None:
        ...
        with self._session_factory() as session:
            for row in SEED_INVESTMENTS:
                ...
                inserted += result.rowcount
            session.commit()
        self.stats["investment_options"] = {
            "attempted": len(SEED_INVESTMENTS),
            "inserted": inserted,
            "skipped_validation": skipped_validation,
        }
        logger.info(
            "investment_options: inserted=%d  skipped_validation=%d",
            inserted,
            skipped_validation,
        )
    ...
    def _seed_transfers(self) -> None:
        ...
        with self._session_factory() as session:
            for tmpl in SEED_TRANSFERS_TEMPLATE:
                ...
                if sender_id is None or receiver_id is None:
                    logger.warning(
                        "SKIP transfer %s — unresolved FK (sender=%s, receiver=%s)",
                        tmpl["reference"],
                        tmpl["_sender"],
                        tmpl["_receiver"],
                    )
        ...
        logger.info(
            "p2p_transfers: inserted=%d  skipped_validation=%d  skipped_fk=%d",
            inserted,
            skipped_validation,
            skipped_fk,
        )
    ...
    def _seed_cards(self) -> None:
        ...
        with self._session_factory() as session:
            for tmpl in SEED_CARDS_TEMPLATE:
                ...
                if holder_id is None:
                    logger.warning(
                        "SKIP virtual_card for %s — unresolved FK", tmpl["_holder"]
                    )
                    skipped_fk += 1
                    continue
                row: dict[str, Any] = {
                ...
                if not _is_valid(
                    "virtual_cards", row, f"{tmpl['_holder']}/{tmpl['card_type']}"
                ):
                    skipped_validation += 1
                    continue
                result = session.execute(
        ...
        logger.info(
            "virtual_cards: inserted=%d  skipped_validation=%d  skipped_fk=%d",
            inserted,
            skipped_validation,
            skipped_fk,
        )
    ...
    def _seed_insights(self) -> None:
        ...
        with self._session_factory() as session:
            for tmpl in SEED_INSIGHTS_TEMPLATE:
                ...
                if account_id is None:
                    logger.warning(
                        "SKIP spending_insight for %s — unresolved FK", tmpl["_account"]
                    )
                    skipped_fk += 1
                    continue
                row: dict[str, Any] = {
        ...
        logger.info(
            "spending_insights: inserted=%d  skipped_validation=%d  skipped_fk=%d",
            inserted,
            skipped_validation,
            skipped_fk,
        )
