...
class TestResearchSeedCatalog:
    ...
    def test_each_seed_has_required_fields(self):
        ...
        for seed in seeds:
            ...
            assert len(seed["acceptance_criteria"]) >= 2, (
                f"Need >=2 criteria: {seed['title']}"
            )
    ...
    def test_descriptions_are_substantial(self):
        ...
        for seed in seeds:
            assert len(seed["description"]) >= 50, (
                f"Description too short: {seed['title']}"
            )
    ...
    def test_difficulty_distribution(self):
        ...
        for diff in ["beginner", "intermediate", "advanced", "expert"]:
            assert counts.get(diff, 0) >= 10, (
                f"Too few {diff} exercises: {counts.get(diff, 0)}"
            )
    ...
    def test_topic_coverage(self):
        ...
        for topic in expected_topics:
            assert any(topic in tag for tag in all_tags), (
                f"Missing topic coverage: {topic}"
            )
    ...
    def test_task_types_set(self):
        ...
        seeds = get_all_seeds()["autoresearch"]
        for seed in seeds:
            assert seed.get("task_type", "") != "", (
                f"Missing task_type: {seed['title']}"
            )
    ...
    def test_seeds_have_metric_info(self):
        ...
        for seed in seeds:
            ...
            assert "metric_direction" in seed, (
                f"Missing metric_direction: {seed['title']}"
            )
