...
def test_get_pending_count():
    ...
    assert q.get_pending_count() == 3, (
        f"Expected 3 pending, got {q.get_pending_count()}"
    )
...
def test_get_next_returns_task():
    ...
    assert task.task_type == "REVIEW_MR", (
        f"Expected task_type 'REVIEW_MR', got '{task.task_type}'."
    )
...
def test_mark_done_removes_from_pending():
    ...
    assert q.get_pending_count() == 1, (
        f"Expected 1 pending after mark_done, got {q.get_pending_count()}"
    )
...
def test_get_status_pending():
    ...
    assert status["status"] == "pending", (
        f"Expected 'pending', got '{status['status']}'."
    )
...
def test_queue_fifo_order():
    ...
    assert task_a.task_id == id_a, (
        f"Expected first task to be A (id={id_a}), got {task_a.task_id}"
    )
    assert task_b.task_id == id_b, (
        f"Expected second task to be B (id={id_b}), got {task_b.task_id}"
    )
    assert task_c.task_id == id_c, (
        f"Expected third task to be C (id={id_c}), got {task_c.task_id}"
    )
...
def test_worker_dispatches_to_analyst():
    ...
    with patch("src.agents.analyst.analyst_agent", mock_analyst):
        q.submit("ANALYZE_LOG", {"log_entry": "ERROR: test dispatch"})
...
def test_worker_handles_exception():
    ...
    q.submit("ANALYZE_LOG", {"log_entry": "trigger exception"})
    task = q.get_next(timeout=1.0)
    assert task is not None
...
def test_mark_done_emits_structured_log_record(caplog):
    ...
    matches = [
        r for r in caplog.records if getattr(r, "event", None) == "task_completed"
    ]
