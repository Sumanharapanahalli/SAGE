...
class TestCDSClassifyInputs:
    ...
    def test_classifies_images(self):
        resp = client.post(
            "/cds/classify-inputs", json={"input_types": ["ct_scan", "mri_image"]}
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_classifies_discrete(self):
        resp = client.post(
            "/cds/classify-inputs",
            json={"input_types": ["blood_pressure_single", "bmi"]},
        )
        assert resp.status_code == 200
        data = resp.json()
...
class TestCDSClassifyOutput:
    ...
    def test_recommendation(self):
        resp = client.post(
            "/cds/classify-output", json={"output_type": "recommendation"}
        )
        assert resp.status_code == 200
    ...
    def test_diagnosis(self):
        resp = client.post(
            "/cds/classify-output", json={"output_type": "definitive_diagnosis"}
        )
        assert resp.status_code == 200
