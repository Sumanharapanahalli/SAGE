...
def load_and_split(cfg: Dict[str, Any], smoke_test: bool = False) -> DatasetDict:
    ...
    try:
        ...
        logger.warning(
            "Could not load HuggingFace dataset (%s); generating synthetic data.", exc
        )
    ...
    logger.info(
        ...
        len(df_val),
        len(df_test),
        df_train[label_col].mean(),
        df_test[label_col].mean(),
    )
        ...
        return Dataset.from_dict(
            {
                ...
                "label": dataframe[label_col].tolist(),
            }
        )
...
def train(cfg: Dict[str, Any], smoke_test: bool = False) -> Dict[str, Any]:
    ...
    with mlflow.start_run(run_name="toxic-bert-finetune") as run:
        ...
        tokenized = tokenize_datasets(
            raw_datasets, tokenizer, cfg["model"]["max_length"]
        )
