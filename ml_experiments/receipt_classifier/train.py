...
def train(
    ...
    # ── 3. Imbalance check ───────────────────────────────────────────────────
    imbalanced = check_class_imbalance(
        y_raw, threshold=training_cfg["imbalance_threshold"]
    )
    ...
    if p95_latency_ms > sla_ms:
        logger.warning(
            "SLA BREACH: p95 latency %.2fms > %dms SLA", p95_latency_ms, sla_ms
        )
    else:
    ...
    cv = StratifiedKFold(
        n_splits=training_cfg["cv_folds"], shuffle=True, random_state=seed
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--n-samples", type=int, default=5_000, help="Synthetic dataset size"
    )
