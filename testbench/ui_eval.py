...
def screenshot(url: str, out: Path) -> Path:
    ...
    shutil.copy(HERE / "drivers" / "screenshot.mjs", target)  # ESM resolves playwright from pw
    try:
        p = subprocess.run([node, str(target), url, str(out)], cwd=pw,
...
def gemini_visual_eval(image: Path, criteria: str, model: str = "gemini-3.5-flash") -> dict:
    ...
    p = subprocess.run([gem, "-m", model], input=prompt, cwd=str(image.parent),
                       capture_output=True, text=True, timeout=180, encoding="utf-8", errors="replace")
    out = (p.stdout or "").strip()
    m = re.search(r"\{.*\}", out, re.DOTALL)
    if not m:
        return {"score": 0, "pass": False, "feedback": f"(no JSON from gemini) {out[:160]}"}
    try:
...
def main(argv=None):
    ...
    ap.add_argument("--criteria", default="consistent design system, accessible (good contrast, "
                    "not colour-only), clear hierarchy, no clashing styles")
    ap.add_argument("--out", default="ui_eval_shot.png", help="where to save the screenshot (with --url)")
