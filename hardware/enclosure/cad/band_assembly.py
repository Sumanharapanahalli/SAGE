...
def build_buckle() -> cq.Workplane:
    ...
    outer = cq.Workplane("XY").box(
        BUCKLE_FRAME_W, BUCKLE_FRAME_D, BUCKLE_FRAME_H, centered=(True, True, False)
    )
    inner = cq.Workplane("XY").box(
        BUCKLE_FRAME_W - 2 * BUCKLE_WALL_T,
        BUCKLE_FRAME_D + 0.1,
...
def build_spring_bar() -> cq.Workplane:
    ...
    return cq.Workplane("YZ").circle(0.75).extrude(26.0).translate((0, -13.0, 0))
...
def build_assembly() -> cq.Workplane:
    ...
    bar = build_spring_bar().translate((-total_l / 2.0 + LUG_HOLE_X, 0, BAND_T / 2.0))
