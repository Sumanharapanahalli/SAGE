...
def _oring_groove_cutter(
    cl_w: float, cl_d: float, g_w: float, g_d: float
) -> cq.Workplane:
    """Return a solid shaped like the rectangular groove channel."""
    g_w / 2.0
    outer_box = cq.Workplane("XY").box(
        cl_w + g_w, cl_d + g_w, g_d, centered=(True, True, False)
    )
    inner_box = cq.Workplane("XY").box(
        cl_w - g_w, cl_d - g_w, g_d + 0.1, centered=(True, True, False)
    )
...
def build_top() -> cq.Workplane:
    ...
    top = top.faces(">Z").workplane().rect(LED_W, LED_D).cutBlind(LED_RECESS)
    # Inner through-hole (light path)
    inner_led_w = LED_W - 2.0 * LED_LIP_W
    inner_led_d = LED_D - 2.0 * LED_LIP_W
    top = top.faces(">Z").workplane().rect(inner_led_w, inner_led_d).cutThruAll()
...
if __name__ == "__main__":
    ...
    print(
        f"  BBox: {bb.xmax - bb.xmin:.2f} x {bb.ymax - bb.ymin:.2f} x {bb.zmax - bb.zmin:.2f} mm"
    )
