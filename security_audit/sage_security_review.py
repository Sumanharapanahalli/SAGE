...
def check_api_key_salt(findings: list[Finding]) -> None:
    ...
    if (
        "sha256" in text.lower()
        and "salt" not in text.lower()
...
def check_audit_log_encryption(findings: list[Finding]) -> None:
    ...
    if (
        "sqlite3" in text
        and "sqlcipher" not in text.lower()
        and "fernet" not in text.lower()
    ):
        findings.append(
            Finding(
                id="ENC-001",
...
def evaluate_acceptance_criteria(findings: list[Finding]) -> list[AcceptanceCriteria]:
    ...
    return [
        ...
        AcceptanceCriteria(
            ...
            detail=by_id.get(
                "AUDIT-001", by_id.get("AUDIT-002", AcceptanceCriteria("", False, ""))
            ).evidence
            if "AUDIT-001" in by_id
...
def build_report(findings: list[Finding]) -> dict[str, Any]:
    ...
    return {
        ...
        "recommendations_priority": {
            "immediate": [f.id for f in findings if f.severity == SEVERITY_CRITICAL],
            "short_term": [f.id for f in findings if f.severity == SEVERITY_HIGH],
            "planned": [
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default="security_audit/sage_security_report.json",
