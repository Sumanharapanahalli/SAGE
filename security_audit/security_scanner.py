...
def _check_audit_append_only() -> tuple[Status, str]:
    ...
    forbidden = re.findall(
        r"\b(UPDATE|DELETE)\s+(?:FROM\s+)?compliance_audit_log\b", text, re.IGNORECASE
    )
    if forbidden:
        return Status.FAIL, f"Mutable SQL on compliance_audit_log found: {forbidden}"
    return (
        Status.PASS,
        "compliance_audit_log is INSERT-only (no UPDATE/DELETE on audit rows)",
    )
...
def _check_phi_encryption_in_transit() -> tuple[Status, str]:
    ...
    if api_file.exists() and _source_contains(
        api_file, r"HTTPSRedirect|ssl_context|TLSv"
    ):
                ...
                findings.append(
                    Finding(
                        ...
                        remediation=(
                            ...
                            if sev_raw
                            else f"No fix available — evaluate mitigation for {v.get('id')}"
                        ),
...
def run_scan(sbom_out: Path, report_out: Path) -> ScanReport:
    ...
    # --- STRIDE checks ---
    logger.info(
        "[1/4] Running STRIDE threat model checks (%d checks)...", len(STRIDE_CHECKS)
    )
    ...
    logger.info(
        "  %s [AUDIT-01] %s",
        "✓" if audit_finding.status == Status.PASS else "✗",
        audit_finding.title,
    )
    ...
    report.summary = {
        ...
        "stride_categories_covered": list(
            {
                f.category
    ...
    logger.info(
        "  Checks : %d total | %d pass | %d fail | %d warn | %d skipped",
        total,
        passed,
        failed,
        warned,
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="SAGE Security Review & Vulnerability Scanner"
    )
    p.add_argument(
        "--sbom-out", default="security_audit/sbom.json", help="SBOM output path"
    )
    p.add_argument(
        "--report-out",
        default="security_audit/audit/scan_report.json",
