...
def check_payment_audit_logging(findings: list[Finding]) -> None:
    ...
    payment_audit = (
        any(kw in api_text.lower() for kw in ["payment", "charge", "transaction"])
...
def check_encryption_at_rest(findings: list[Finding]) -> None:
    ...
    has_field_enc = any(
        kw in text.lower() for kw in ["fernet", "sqlcipher", "encrypt", "aes"]
    )
    config_path = ROOT / "config" / "config.yaml"
    cfg_text = _read(config_path)
...
def check_tokenization(findings: list[Finding]) -> None:
    ...
    has_tokenization = any(
        kw in text.lower()
        for kw in ["tokenize", "vault_token", "payment_token", "tok_"]
    )
...
def check_idempotency(findings: list[Finding]) -> None:
    ...
    has_idempotency = any(
        kw in text.lower()
        for kw in ["idempotency_key", "idempotency-key", "x-idempotency"]
    )
...
def evaluate_pci_controls(findings: list[Finding]) -> list[PCIDSSControl]:
    ...
    return [
        PCIDSSControl(
            ...
            evidence="Findings: "
            + ", ".join({"PAN-001", "PAN-002", "ENC-001", "TOKEN-001"} & finding_ids)
            if {"PAN-001", "PAN-002", "ENC-001", "TOKEN-001"} & finding_ids
            else "No PAN storage violations detected in codebase scan",
            finding_ids=list(
                {"PAN-001", "PAN-002", "ENC-001", "TOKEN-001"} & finding_ids
            ),
        ),
        PCIDSSControl(
        ...
        PCIDSSControl(
            ...
            status="FAIL"
            if {"INJECT-001", "HDR-001", "SBOM-001", "IDEMPOTENCY-001", "CORS-001"}
            & finding_ids
            else "PASS",
            evidence="Findings: "
            + ", ".join(
                {"INJECT-001", "HDR-001", "SBOM-001", "IDEMPOTENCY-001", "CORS-001"}
                & finding_ids
            )
            if {"INJECT-001", "HDR-001", "SBOM-001", "IDEMPOTENCY-001", "CORS-001"}
            & finding_ids
        ...
        PCIDSSControl(
            ...
            evidence="Findings: AUDIT-001"
            if "AUDIT-001" in finding_ids
...
def evaluate_acceptance_criteria(findings: list[Finding]) -> list[AcceptanceCriteria]:
    ...
    crit_high = [
        f
        for f in findings
        if f.severity in (SEV_CRITICAL, SEV_HIGH) and f.id not in ("AUDIT-002",)
    ]
    ...
    return [
        ...
        AcceptanceCriteria(
            ...
            if crit_high
            else "PASSED — no critical/high vulnerabilities",
        ),
        AcceptanceCriteria(
            name="SBOM generated",
            passed=sbom_exists,
            detail=(
                ...
                if sbom_exists
                else "SBOM not yet generated — run sbom generation step"
            ),
        ),
        AcceptanceCriteria(
            ...
            detail=(
                ...
                if not enc_ok
                else "PASSED — no plaintext cardholder data findings"
            ),
        ),
        AcceptanceCriteria(
            ...
            detail=(
                ...
                if not audit_ok
                else "PASSED — audit logging verified"
            ),
        ),
        AcceptanceCriteria(
            ...
            detail=(
                ...
                if pci_saq_exists
                else "PCI DSS SAQ not yet completed"
            ),
        ),
        AcceptanceCriteria(
            ...
            detail=(
                "FAILED — encryption gaps: "
                + ", ".join(x for x in ["ENC-001", "TLS-001", "TLS-002"] if x in by_id)
...
def build_report(
    ...
    return {
        ...
        "executive_summary": {
            ...
            "pci_dss_controls_passing": sum(
                1 for c in pci_controls if c.status == "PASS"
            ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="SAGE payment gateway security scanner"
    )
    parser.add_argument(
        "--output",
    ...
    print(f"\n{'=' * 65}")
    print(
        f"  Payment Gateway Security Review — {report['report_metadata']['generated_at'][:10]}"
    )
    print(f"{'=' * 65}")
    ...
    print(
        f"  PCI controls pass : {summary['pci_dss_controls_passing']}/{summary['pci_dss_controls_total']}"
    )
    print(
        f"  Criteria pass     : {'YES' if summary['acceptance_criteria_passed'] else 'NO'}"
    )
