...
exports.showBackendProposalDetail = showBackendProposalDetail;
exports.approveBackendProposal = approveBackendProposal;
exports.rejectBackendProposal = rejectBackendProposal;
...
function showBackendProposalDetail(proposal) {
    const ch = outputChannel();
    ch.appendLine('');
    ch.appendLine('━'.repeat(70));
...
async function approveBackendProposal(client, proposal, refresh) {
    const confirm = await vscode.window.showWarningMessage(`Approve: ${proposal.description}?`, { modal: true }, 'Approve');
    if (confirm !== 'Approve')
        return;
    try {
...
async function rejectBackendProposal(client, proposal, refresh) {
