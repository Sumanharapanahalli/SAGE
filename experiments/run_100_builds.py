...
REGISTRY = os.path.join(
    os.path.dirname(__file__), "..", "test-solutions", "solutions_registry.csv"
)
...
def _generate_user_stories(desc, domain):
    ...
    story_patterns = [
        ...
        (
            "export",
            "As a user, I want to export data to CSV/PDF so that I can share it externally",
        ),
    ]
    ...
    if len(stories) < 5:
        stories.append(
            f"As a user, I want core {domain} functionality so that the primary use case is met"
        )
...
def simulate_se(sol, po_result):
    ...
    # Get HITL level
    hitl_level = (
        "strict" if sol.get("regulation_level", "").lower() == "strict" else "standard"
    )
    ...
    try:
        ...
        if domain == "automotive":
            ...
            if any(k in name for k in ("adas", "autonomous", "v2x", "battery")):
                asil_result = functional_safety.classify_asil(
                    "S3", "E4", "C3"
                )  # ASIL D
        ...
        if domain in ("medtech", "iot"):
            ...
            if any(k in name for k in ("insulin", "surgical", "imaging", "industrial")):
                ...
                k in name
                for k in ("patient_monitoring", "fall_detection", "cold_chain")
            ):
        ...
        if domain == "medtech":
            ...
            if any(k in name for k in ("insulin", "surgical")):
                iec_result = functional_safety.classify_iec62304(
                    "death_possible"
                )  # Class C
            elif any(
                k in name for k in ("patient_monitoring", "fall_detection", "imaging")
            ):
                iec_result = functional_safety.classify_iec62304(
                    "injury_possible"
                )  # Class B
...
def _generate_fmea_entries(sol):
    ...
    solution_fmea = {
        ...
        "fall_detection": [
            ...
            },
            {
                ...
                "failure_mode": "SMS gateway timeout",
                "effect": "Caregiver not notified",
                "severity": 10,
                "occurrence": 2,
                "detection": 3,
            },
            {
                "component": "GPS Tracker",
                "failure_mode": "No satellite lock",
                "effect": "Unknown patient location",
                "severity": 8,
                "occurrence": 4,
                "detection": 5,
            },
            {
                "component": "Battery Manager",
                "failure_mode": "Premature shutdown",
                "effect": "Device offline during emergency",
                "severity": 10,
                "occurrence": 2,
                "detection": 4,
            },
        ...
        "insulin_pump": [
            {
                "component": "CGM Interface",
                "failure_mode": "Stale glucose reading",
                "effect": "Incorrect dosing calculation",
                "severity": 10,
                "occurrence": 3,
                "detection": 3,
            },
            {
                "component": "Dosing Algorithm",
                "failure_mode": "Overflow in calculation",
                "effect": "Dangerous insulin overdose",
                "severity": 10,
                "occurrence": 1,
                "detection": 2,
            },
            {
                "component": "Pump Motor Driver",
                "failure_mode": "Stuck valve",
                "effect": "No insulin delivery",
                "severity": 9,
                "occurrence": 2,
                "detection": 4,
            },
            {
                "component": "Bluetooth Link",
                "failure_mode": "Connection drop",
                "effect": "Loss of monitoring visibility",
                "severity": 7,
                "occurrence": 4,
                "detection": 3,
            },
        ...
        "telehealth": [
            {
                ...
                "failure_mode": "Codec failure",
                "effect": "Consultation interrupted",
                "severity": 7,
                "occurrence": 3,
                "detection": 2,
            },
            {
                "component": "E-Prescription Module",
                "failure_mode": "Drug interaction missed",
                "effect": "Adverse drug event",
                "severity": 10,
                "occurrence": 2,
                "detection": 4,
            },
            {
                ...
                "failure_mode": "HL7 parse error",
                "effect": "Patient data not synced",
                "severity": 8,
                "occurrence": 3,
                "detection": 3,
            },
        ...
        "surgical_robot": [
            {
                "component": "Haptic Controller",
                "failure_mode": "Force feedback lag",
                "effect": "Surgeon loses tissue feel",
                "severity": 10,
                "occurrence": 2,
                "detection": 2,
            },
            {
                ...
                "failure_mode": "Frame drop below 30fps",
                "effect": "Depth perception compromised",
                "severity": 9,
                "occurrence": 3,
                "detection": 3,
            },
            {
                "component": "Instrument Tracker",
                "failure_mode": "Position error >1mm",
                "effect": "Unintended tissue damage",
                "severity": 10,
                "occurrence": 2,
                "detection": 3,
            },
            {
                ...
                "failure_mode": "E-stop signal ignored",
                "effect": "Uncontrolled instrument motion",
                "severity": 10,
                "occurrence": 1,
                "detection": 1,
            },
        ...
        "patient_monitoring": [
            {
                "component": "Vital Signs Display",
                "failure_mode": "Stale data shown",
                "effect": "Clinician acts on old data",
                "severity": 9,
                "occurrence": 3,
                "detection": 4,
            },
            {
                ...
                "failure_mode": "Alarm suppressed",
                "effect": "Critical event unnoticed",
                "severity": 10,
                "occurrence": 2,
                "detection": 3,
            },
            {
                "component": "Trend Calculator",
                "failure_mode": "Moving average overflow",
                "effect": "False trend indication",
                "severity": 7,
                "occurrence": 2,
                "detection": 4,
            },
        ...
        "clinical_trial": [
            {
                "component": "Randomization Engine",
                "failure_mode": "Bias in allocation",
                "effect": "Trial integrity compromised",
                "severity": 10,
                "occurrence": 2,
                "detection": 5,
            },
            {
                "component": "eCRF Module",
                "failure_mode": "Data field truncation",
                "effect": "Regulatory submission data loss",
                "severity": 9,
                "occurrence": 3,
                "detection": 4,
            },
            {
                "component": "Adverse Event Reporter",
                "failure_mode": "Missed SAE deadline",
                "effect": "Regulatory violation",
                "severity": 10,
                "occurrence": 2,
                "detection": 3,
            },
        ...
        "medical_imaging": [
            {
                ...
                "failure_mode": "Pixel data corruption",
                "effect": "Misdiagnosis from artifact",
                "severity": 10,
                "occurrence": 2,
                "detection": 3,
            },
            ...
            },
            {
                ...
                "failure_mode": "Wrong patient ID",
                "effect": "Report attached to wrong patient",
                "severity": 9,
                "occurrence": 2,
                "detection": 2,
            },
        ...
        "adas_perception": [
            {
                "component": "LiDAR Processor",
                "failure_mode": "Point cloud dropout",
                "effect": "Pedestrian not detected",
                "severity": 10,
                "occurrence": 2,
                "detection": 3,
            },
            {
                ...
                "failure_mode": "Lens flare saturation",
                "effect": "Lane markings lost",
                "severity": 8,
                "occurrence": 4,
                "detection": 4,
            },
            {
                "component": "Radar Tracker",
                "failure_mode": "Ghost target",
                "effect": "Phantom braking event",
                "severity": 7,
                "occurrence": 3,
                "detection": 3,
            },
            {
                ...
                "failure_mode": "Timestamp misalignment",
                "effect": "Object position error",
                "severity": 9,
                "occurrence": 2,
                "detection": 3,
            },
        ...
        "battery_management": [
            {
                "component": "Cell Balancer",
                "failure_mode": "Bypass transistor stuck ON",
                "effect": "Cell overdischarge / thermal runaway",
                "severity": 10,
                "occurrence": 2,
                "detection": 3,
            },
            {
                "component": "SOC Estimator",
                "failure_mode": "Coulomb counting drift",
                "effect": "Incorrect range estimate",
                "severity": 6,
                "occurrence": 4,
                "detection": 4,
            },
            {
                "component": "Thermal Manager",
                "failure_mode": "Coolant pump failure",
                "effect": "Battery overtemperature",
                "severity": 10,
                "occurrence": 2,
                "detection": 2,
            },
            {
                "component": "CAN Reporter",
                "failure_mode": "DTC suppressed",
                "effect": "Dashboard shows no fault",
                "severity": 8,
                "occurrence": 2,
                "detection": 4,
            },
        ...
        "infotainment": [
            {
                ...
                "failure_mode": "Audio underrun",
                "effect": "Audio dropout",
                "severity": 3,
                "occurrence": 5,
                "detection": 2,
            },
            {
                "component": "Navigation Engine",
                "failure_mode": "Map tile cache miss",
                "effect": "Blank map region",
                "severity": 5,
                "occurrence": 3,
                "detection": 3,
            },
            {
                ...
                "failure_mode": "Pairing loop",
                "effect": "Phone disconnects repeatedly",
                "severity": 4,
                "occurrence": 4,
                "detection": 3,
            },
        ...
        "fleet_telematics": [
            {
                ...
                "failure_mode": "Position drift in urban canyon",
                "effect": "Incorrect vehicle location",
                "severity": 6,
                "occurrence": 4,
                "detection": 4,
            },
            {
                "component": "OBD Data Logger",
                "failure_mode": "PID decode error",
                "effect": "Wrong fuel efficiency metric",
                "severity": 5,
                "occurrence": 3,
                "detection": 4,
            },
            {
                "component": "Geofence Engine",
                "failure_mode": "Polygon overflow",
                "effect": "Missed boundary violation",
                "severity": 7,
                "occurrence": 2,
                "detection": 3,
            },
        ...
        "autonomous_parking": [
            {
                ...
                "failure_mode": "Cross-talk interference",
                "effect": "Phantom obstacle detected",
                "severity": 7,
                "occurrence": 3,
                "detection": 3,
            },
            {
                ...
                "failure_mode": "Deadlock in tight space",
                "effect": "Vehicle stuck mid-maneuver",
                "severity": 6,
                "occurrence": 3,
                "detection": 4,
            },
            {
                "component": "Steering Actuator",
                "failure_mode": "Angle sensor offset",
                "effect": "Vehicle drifts from planned path",
                "severity": 9,
                "occurrence": 2,
                "detection": 2,
            },
        ...
        "smart_home": [
            ...
            },
            {
                ...
                "failure_mode": "Infinite trigger loop",
                "effect": "Device flapping on/off",
                "severity": 6,
                "occurrence": 3,
                "detection": 4,
            },
            {
                ...
                "failure_mode": "Wake word false positive",
                "effect": "Unintended device activation",
                "severity": 4,
                "occurrence": 5,
                "detection": 3,
            },
        ...
        "industrial_iot": [
            {
                "component": "PLC Gateway",
                "failure_mode": "Modbus CRC error",
                "effect": "Incorrect sensor reading",
                "severity": 8,
                "occurrence": 3,
                "detection": 3,
            },
            {
                "component": "SCADA Bridge",
                "failure_mode": "Buffer overflow",
                "effect": "Control system compromise",
                "severity": 10,
                "occurrence": 2,
                "detection": 4,
            },
            {
                ...
                "failure_mode": "Concept drift",
                "effect": "False maintenance alert",
                "severity": 5,
                "occurrence": 4,
                "detection": 5,
            },
        ...
        "agriculture": [
            {
                ...
                "failure_mode": "Electrode corrosion",
                "effect": "Over-irrigation",
                "severity": 5,
                "occurrence": 4,
                "detection": 5,
            },
            {
                ...
                "failure_mode": "Anemometer stuck",
                "effect": "Incorrect wind speed",
                "severity": 4,
                "occurrence": 3,
                "detection": 4,
            },
            {
                "component": "Irrigation Controller",
                "failure_mode": "Valve stuck open",
                "effect": "Water waste / crop flooding",
                "severity": 7,
                "occurrence": 3,
                "detection": 3,
            },
        ...
        "cold_chain": [
            {
                "component": "Temperature Logger",
                "failure_mode": "Sensor out of calibration",
                "effect": "Undetected excursion",
                "severity": 9,
                "occurrence": 3,
                "detection": 4,
            },
            {
                "component": "Alert Gateway",
                "failure_mode": "Cellular dead zone",
                "effect": "Delayed excursion notification",
                "severity": 8,
                "occurrence": 3,
                "detection": 4,
            },
            {
                ...
                "failure_mode": "Time zone mismatch",
                "effect": "Audit log timestamps wrong",
                "severity": 7,
                "occurrence": 2,
                "detection": 3,
            },
    ...
    domain_fallback = {
        "medtech": [
            {
                ...
                "failure_mode": "Data validation bypass",
                "effect": "Invalid clinical data stored",
                "severity": 9,
                "occurrence": 3,
                "detection": 4,
            },
            {
                ...
                "failure_mode": "Misleading status indicator",
                "effect": "Clinician makes wrong decision",
                "severity": 8,
                "occurrence": 3,
                "detection": 3,
            },
            {
                ...
                "failure_mode": "HL7/FHIR mapping error",
                "effect": "Patient data mismatch",
                "severity": 9,
                "occurrence": 2,
                "detection": 4,
            },
        ...
        "automotive": [
            {
                "component": "ECU Interface",
                "failure_mode": "Message dropout on CAN",
                "effect": "Loss of telemetry data",
                "severity": 7,
                "occurrence": 3,
                "detection": 3,
            },
            {
                ...
                "failure_mode": "Watchdog not triggered",
                "effect": "Silent system hang",
                "severity": 9,
                "occurrence": 2,
                "detection": 3,
            },
            {
                ...
                "failure_mode": "DTC suppressed",
                "effect": "Fault not reported to driver",
                "severity": 8,
                "occurrence": 2,
                "detection": 4,
            },
        ...
        "iot": [
            {
                ...
                "failure_mode": "Connection loss",
                "effect": "Data gap in telemetry",
                "severity": 5,
                "occurrence": 5,
                "detection": 3,
            },
            {
                ...
                "failure_mode": "Partial update",
                "effect": "Bricked device",
                "severity": 8,
                "occurrence": 2,
                "detection": 4,
            },
...
def _generate_summary(results):
    ...
    lines.append(
        f"| **TOTAL** | **{len(results)}** | **{total_agents / len(results):.0f}** | "
...
def _generate_domain_section(domain, results):
    ...
    for r in results:
        ...
        for t in orch["tasks"]:
            criteria_str = (
                f" — Criteria: {'; '.join(t['acceptance_criteria'][:2])}"
                if t["acceptance_criteria"]
                else ""
            )
            lines.append(
                f"- Step {t['step']}: `{t['task_type']}` → `{t['agent_role']}`{criteria_str}"
            )
        ...
        if r.get("safety"):
            ...
            if safety.get("fmea"):
                fmea = safety["fmea"]
                max_rpn = max(
                    (e.get("rpn", 0) for e in fmea.get("entries", [])), default=0
                )
                lines.append(
                    f"- FMEA: {len(fmea.get('entries', []))} failure modes analyzed, max RPN={max_rpn}"
                )
            if safety.get("asil"):
                lines.append(
                    f"- ASIL Classification: **{safety['asil'].get('asil', '?')}**"
                )
            if safety.get("sil"):
                lines.append(
                    f"- SIL Classification: **SIL {safety['sil'].get('sil', '?')}**"
                )
            if safety.get("iec62304"):
                lines.append(
                    f"- IEC 62304 Safety Class: **{safety['iec62304'].get('safety_class', '?')}**"
                )
...
def _generate_cross_domain(results):
    ...
    standard_domains = [
        r
        for r in results
        if r["orchestrator"]["domain_rules"]["hitl_level"] == "standard"
    ]

    strict_avg = (
        sum(r["orchestrator"]["agent_count"] for r in strict_domains)
        / len(strict_domains)
        if strict_domains
        else 0
    )
    standard_avg = (
        sum(r["orchestrator"]["agent_count"] for r in standard_domains)
        / len(standard_domains)
        if standard_domains
        else 0
    )
...
def _generate_agent_heatmap(results):
    ...
    domains = sorted(set(r["domain"] for r in results))
    all_agents = sorted(
        set(a for r in results for a in r["orchestrator"]["agents_activated"])
    )
...
def main():
    ...
    for i, sol in enumerate(registry, 1):
        ...
        logger.info(
            "  → %d agents, %d tasks, %d standards (%.1fs)",
            orch_result["agent_count"],
            orch_result["task_count"],
            len(se_result["standards"]),
            elapsed,
        )
