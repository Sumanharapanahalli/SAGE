  ...
  return (
    <>
      ...
      >
        ...
        >
          <span className="text-xs font-semibold uppercase tracking-widest" style={{ color: '#52525b' }}>Issue Detail</span>
          <button onClick={onClose} style={{ color: '#71717a' }}>
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {/* Title */}
          <div>
            <div className="text-xs font-medium mb-1" style={{ color: '#71717a' }}>Title</div>
            <div className="text-sm font-medium" style={{ color: '#f4f4f5' }}>{issue.title}</div>
          </div>
          ...
          {/* Scope */}
          <div>
            <div className="text-xs font-medium mb-1" style={{ color: '#71717a' }}>Scope</div>
            <ScopeBadge scope={issue.scope} />
          </div>

          {/* Description */}
          <div>
            <div className="text-xs font-medium mb-1" style={{ color: '#71717a' }}>Description</div>
            <p className="text-sm whitespace-pre-wrap" style={{ color: '#a1a1aa' }}>{issue.description}</p>
          </div>

          {/* Proposed solution / AI analysis */}
          {issue.proposed_solution && (
            <div>
              <div className="text-xs font-medium mb-1" style={{ color: '#71717a' }}>Reviewer note / AI analysis</div>
              <p className="text-sm whitespace-pre-wrap" style={{ color: '#a1a1aa' }}>{issue.proposed_solution}</p>
            </div>
          )}

          {/* Created */}
          <div>
            <div className="text-xs font-medium mb-1" style={{ color: '#71717a' }}>Created</div>
            <div className="text-xs" style={{ color: '#52525b' }}>{timeAgo(issue.created_at)}</div>
          </div>
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold" style={{ color: '#f4f4f5' }}>New Issue</span>
        <button onClick={onClose} style={{ color: '#71717a' }}><X size={14} /></button>
      </div>
      ...
      <div className="flex gap-2">
        <button
          ...
          className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5"
          style={{ backgroundColor: '#3f3f46', color: '#f4f4f5', opacity: isPending || !title.trim() || !desc.trim() ? 0.5 : 1 }}
        >
          {isPending ? <><Loader2 size={11} className="animate-spin" /> Submitting…</> : 'Submit'}
        </button>
        <button onClick={onClose} className="text-xs px-3 py-1.5" style={{ color: '#71717a' }}>Cancel</button>
      </div>
    </div>
  )
}
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1">
            {STATUS_FILTERS.map(f => (
              <button
                ...
                style={{
                  backgroundColor: statusFilter === f.value ? '#3f3f46' : 'transparent',
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5"
            style={{ backgroundColor: '#3f3f46', color: '#f4f4f5', border: '1px solid #52525b' }}
          >
            <Plus size={12} />
            New Issue
          </button>
            ...
            {filtered.map((issue, idx) => (
              <button
                ...
                style={{
                  ...
                  gap: '12px',
                  alignItems: 'center',
                  borderBottom: idx < filtered.length - 1 ? '1px solid #27272a' : 'none',
              ...
              >
                ...
                <span className="text-xs text-right" style={{ color: '#52525b' }}>
