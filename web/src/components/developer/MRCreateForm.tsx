    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        ...
        <div>
          ...
          <input type="number" value={issueIid} onChange={(e) => setIssueIid(e.target.value)}
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="42" />
        </div>
      </div>
      <div className="mb-3">
        <label className="text-xs text-gray-500 mb-1 block">Branch (optional)</label>
        <input type="text" value={branch} onChange={(e) => setBranch(e.target.value)}
          className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="sage-ai/42-fix-sensor" />
      </div>
      ...
      {result && !result.error && (
        <div className="mb-3 p-3 bg-green-50 border border-green-200 rounded-lg text-sm">
          MR created: <a href={result.mr_url} target="_blank" rel="noreferrer" className="text-green-700 underline">{result.mr_title}</a>
        </div>
      )}
