    ...
    if not firmware_path:
        ...
        return {
            **state,
            "flash_result": {
                "success": True,
                "output": "No firmware to flash — skipped",
                "skipped": True,
            },
        }
...
def collect_evidence(state: HILState) -> HILState:
    ...
    logger.info(
        "Evidence collected: %d test results, %d compliance flags",
        len(evidence["test_evidence"]),
        len(flags),
    )
...
def generate_report(state: HILState) -> HILState:
    ...
    failed_ids = {
        r.get("test_id")
        for r in suite_results.get("results", [])
        if r.get("verdict") in ("FAIL", "ERROR", "BLOCKED")
    }
    blocking_failures = []
    ...
    report["regulatory_gate_passed"] = (
        len(blocking_failures) == 0 and suite_results.get("failed", 0) == 0
    )
...
def submit_evidence(state: HILState) -> HILState:
    ...
    if not approved:
        logger.warning(
            "submit_evidence called but approved=False — evidence NOT submitted"
        )
        return {
            **state,
