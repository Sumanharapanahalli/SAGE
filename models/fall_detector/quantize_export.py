...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--n-calib", type=int, default=1000, help="Number of calibration windows"
    )
    return p.parse_args()
...
def get_calibration_data(
    ...
    if calib_path and calib_path.exists():
        ...
        logger.warning(
            "Calibration file has only %d samples, need %d — using all", len(data), n
        )
...
def quantize_to_int8(
    ...
    logger.info(
        "Running INT8 quantization with %d calibration windows …", len(calib_data)
    )
...
def export_fp32(model_path: Path, output_dir: Path) -> Path:
    ...
    out_path.write_bytes(tflite_model)
    logger.info(
        "Saved FP32 TFLite model → %s (%.1f KB)", out_path, len(tflite_model) / 1024
    )
    return out_path
...
if __name__ == "__main__":
    ...
    logger.info(
        "Estimated latency at 80 MHz: %.2f ms  (target ≤20 ms)  [%s]",
