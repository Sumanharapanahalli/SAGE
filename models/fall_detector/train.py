...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--sisfall", type=Path, default=None, help="Root directory of SisFall dataset"
    )
    p.add_argument(
        "--output-dir",
        type=Path,
...
def train(args: argparse.Namespace) -> dict:
    ...
    logger.info(
        "Class weights: ADL=%.3f  Fall=%.3f", class_weights[0], class_weights[1]
    )
    ...
    logger.info(
        "Parameters: %d  (~%s KB INT8)", pinfo["total_params"], pinfo["int8_size_kb"]
    )
    ...
    # ── 5. Training ───────────────────────────────────────────────────────────
    logger.info(
        "=== Phase 3: Training (max %d epochs, batch %d) ===",
        args.epochs,
        args.batch_size,
    )
    t0 = time.time()
...
def _log_metrics(m: dict) -> None:
    ...
    logger.info("  TP=%d  TN=%d  FP=%d  FN=%d", m["tp"], m["tn"], m["fp"], m["fn"])
...
def _assert_acceptance_criteria(metrics: dict, pinfo: dict) -> None:
    ...
    if metrics["sensitivity"] < 0.95:
        failures.append(f"FAIL sensitivity={metrics['sensitivity']:.4f} < 0.95")
    if metrics["specificity"] < 0.90:
        failures.append(f"FAIL specificity={metrics['specificity']:.4f} < 0.90")
    if pinfo["int8_size_kb"] > 100.0:
        failures.append(f"FAIL model size={pinfo['int8_size_kb']} KB > 100 KB")
    if metrics["fp_per_day_est"] > 2.0:
        logger.warning(
            "WARNING fp/day=%.2f > 2.0 — threshold tuning may be required",
    ...
    if failures:
        for f in failures:
            logger.error(f)
        raise ValueError("Acceptance criteria NOT met:\n" + "\n".join(failures))
