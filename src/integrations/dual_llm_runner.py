...
def score_confidence(output: dict, task_type: str = "") -> float:
    ...
    root_cause = str(
        output.get("root_cause_hypothesis", output.get("root_cause", ""))
    ).lower()
...
class DualLLMRunner:
    ...
    def generate(self, prompt: str, system_prompt: str, task_type: str = "") -> str:
        ...
        if strategy == "teacher_only":
            return self._teacher_generate(
                prompt, system_prompt, observe=True, task_type=task_type
            )
            ...
            return self._teacher_generate(
                prompt, system_prompt, observe=False, task_type=task_type
            )
    ...
    def _teacher_generate(
        self, prompt: str, system_prompt: str, observe: bool = True, task_type: str = ""
    ) -> str:
    ...
    def _student_first(self, prompt: str, system_prompt: str, task_type: str) -> str:
        ...
        # Escalate to teacher
        self.logger.info(
            "Escalating %s to teacher (confidence=%.2f < %.2f)",
            task_type,
            confidence,
            self.confidence_threshold,
        )
        teacher_result = self._teacher_generate(
            prompt, system_prompt, observe=False, task_type=task_type
        )
    ...
    def _parallel_compare(self, prompt: str, system_prompt: str, task_type: str) -> str:
        ...
        def run_student():
            try:
                results["student"] = self._student_provider.generate(
        ...
        def run_teacher():
            try:
                results["teacher"] = self._teacher_generate(
                    prompt, system_prompt, observe=False, task_type=task_type
                )
            except Exception as e:
            ...
            try:
                with open(path) as f:
                    for line in f:
                        ...
                        teacher_out = entry.get("teacher_output") or entry.get(
                            "teacher", ""
                        )
                        if not teacher_out:
                            continue
