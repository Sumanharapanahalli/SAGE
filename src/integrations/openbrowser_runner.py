...
class OpenBrowserRunner(BaseRunner):
    ...
    def _check_gstack(self) -> bool:
        ...
        if self._gstack_available is None:
            ...
            if self._gstack_available:
                ...
                self.logger.info(
                    "gstack not installed — browser tests will use LLM simulation"
                )
    ...
    def _execute_with_gstack(
        self, run_id: str, task_type: str, description: str, url: str
    ) -> RunResult:
        ...
        results.append(
            f"Navigation: {'OK' if nav['success'] else 'FAILED'} ({nav['duration_ms']}ms)"
        )
        if not nav["success"]:
        ...
        failed_requests = [
            line
            for line in network["output"].split("\n")
        ...
        if screenshot["success"]:
            screenshots.append(
                {"path": screenshot_path, "type": "screenshot", "size": 0}
            )
        ...
        if perf["success"]:
            try:
                ...
                if isinstance(timing, dict):
                    load_time = timing.get("loadEventEnd", 0) - timing.get(
                        "navigationStart", 0
                    )
                    metrics["page_load_ms"] = load_time
                    metrics["dom_ready_ms"] = timing.get(
    ...
    def _execute_with_llm(
        ...
        try:
            ...
            if start >= 0 and end > start:
                ...
                metrics["a11y_violations"] = len(
                    parsed.get("accessibility", {}).get("violations", [])
                )
    ...
    def get_workflow(self):
        return [
            {"step": 1, "name": "navigate", "description": "Navigate to target URL"},
            {
                "step": 2,
                "name": "snapshot",
                "description": "Capture interactive snapshot with element refs",
            },
            {
                "step": 3,
                "name": "interact",
                "description": "Test user flows (click, fill, verify)",
            },
            {
                "step": 4,
                "name": "audit",
                "description": "Check console errors, network, accessibility",
            },
            {"step": 5, "name": "screenshot", "description": "Capture visual evidence"},
            {
                "step": 6,
                "name": "report",
                "description": "Generate QA report with findings",
            },
        ]
        ...
        exercises = {
            ...
            "expert": [
                ...
                Ex(
                    ...
                    acceptance_criteria=[
                        "10-tab test completed",
                        "Memory per tab tracked",
    ...
    def grade_exercise(self, exercise, result):
        ...
        if qa_hits >= 5:
            ...
            hints.append(
                "Include systematic test steps with assertions and verifications"
            )
        ...
        if browser_hits >= 4:
            ...
            hints.append(
                "Demonstrate actual browser interactions (navigate, click, fill, verify)"
            )
        ...
        if sum(1 for k in a11y_kws if k in output_lower) >= 2:
            ...
            hints.append(
                "Include accessibility checks (WCAG, ARIA, contrast, keyboard nav)"
            )
