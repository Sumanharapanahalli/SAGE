...
class RoleGenerator:
    def generate_system_prompt(
        ...
        severity_labels = (
            "RED | AMBER | GREEN" if is_technical else "CRITICAL | HIGH | MEDIUM | LOW"
        )

        resp_text = (
            "\n".join(f"- {r}" for r in responsibilities)
            if responsibilities
            else "- Execute responsibilities for this role with high quality"
        )
...
Return ONLY the system prompt text. Under 400 words."""
        ...
        try:
            ...
            return self._template_system_prompt(
                job_title, department, responsibilities, severity_labels
            )

    def _template_system_prompt(
        self, job_title, department, responsibilities, severity_labels
    ):
        resp_text = (
            "\n".join(f"- {r}" for r in responsibilities)
            if responsibilities
            else "- Execute tasks within your domain with high quality"
        )
...
Always output JSON with: summary, analysis, recommendations, next_steps, severity ({severity_labels}), confidence (HIGH|MEDIUM|LOW)"""
    ...
    def add_role_to_yaml(
        self, solution_name: str, role_id: str, role_data: dict
    ) -> str:
        """Add a new role to solutions/<solution_name>/prompts.yaml."""
        path = _prompts_yaml_path(solution_name)
        if not os.path.exists(path):
            raise FileNotFoundError(
                f"prompts.yaml not found for solution: {solution_name}"
            )
        ...
        return yaml.dump(
            {role_id: role_data}, default_flow_style=False, allow_unicode=True
        )
    ...
    def update_role_in_yaml(
        self, solution_name: str, role_id: str, updates: dict
    ) -> bool:
