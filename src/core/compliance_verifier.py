...
class ComplianceVerifier:
    ...
    def verify_iec62304(self, project_data: dict) -> List[dict]:
        ...
        results.append(
            self._add_result(
                ...
                passed=len(reqs) > 0
                and all(r.get("acceptance_criteria") for r in reqs),
        ...
        results.append(
            self._add_result(
                ...
                passed=all(r.get("verification_method") for r in reqs)
                if reqs
                else False,
        ...
        traced_items = [
            t for t in trace_data if t.get("traces_to") or t.get("traced_from")
        ]
        results.append(
            self._add_result(
                ...
                passed=len(trace_data) > 0
                and len(traced_items) > len(trace_data) * 0.5,
        ...
        results.append(
            self._add_result(
                ...
                passed=bool(changes)
                or bool(project_data.get("change_control_enabled")),
                evidence=f"{len(changes)} change requests tracked"
                if changes
                else "No change control data",
    ...
    def verify_iso26262(self, project_data: dict) -> List[dict]:
        ...
        results.append(
            self._add_result(
                ...
                passed=bool(
                    project_data.get("change_requests")
                    or project_data.get("change_control_enabled")
                ),
                evidence="Change management: active"
                if project_data.get("change_requests")
                else "Not configured",
    ...
    def verify_en50128(self, project_data: dict) -> List[dict]:
        ...
        results.append(
            self._add_result(
                ...
                passed=bool(
                    project_data.get("architecture") and project_data.get("sil_level")
                ),
    ...
    def verify_21cfr_part11(self, project_data: dict) -> List[dict]:
        ...
        results.append(
            self._add_result(
                ...
                if project_data.get("audit_trail_active")
                else "Not verified",
    ...
    def verify_all(self, project_data: dict, standards: List[str] = None) -> dict:
        ...
        return {
            ...
            "compliance_pct": round(passed_checks / total_checks * 100, 1)
            if total_checks
            else 0,
