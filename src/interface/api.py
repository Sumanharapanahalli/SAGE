...
app.include_router(_collective_intelligence_router)
app.include_router(_orchestrator_router)
app.include_router(_constitution_router)
...
_ORG_TEMPLATES_CACHE = None


def _get_org_templates() -> list:
    """Load onboarding org templates from config/org_templates.yaml (cached).

    Kept as data OUTSIDE src/ so the framework stays domain-blind (SOUL.md);
    add or edit templates in the YAML with no code change. Returns [] if the
    file is missing or unreadable rather than failing the onboarding endpoint.
    """
    ...
    if _ORG_TEMPLATES_CACHE is None:
        ...
        try:
            ...
            _ORG_TEMPLATES_CACHE = data.get("templates", [])
        except Exception:
            logger.error("Failed to load org_templates.yaml from %s", path)
            _ORG_TEMPLATES_CACHE = []
    return _ORG_TEMPLATES_CACHE
...
async def get_org_templates():
    ...
    return {"templates": _get_org_templates()}
