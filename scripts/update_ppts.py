...
def add_footer(slide):
    ...
    rect = slide.shapes.add_shape(
        MSO_SHAPE.RECTANGLE, 0, FOOTER_BAR_TOP, SLIDE_W, FOOTER_BAR_H
    )
    rect.fill.solid()
    rect.fill.fore_color.rgb = BG_DARK
    rect.line.fill.background()
    ...
    tb = slide.shapes.add_textbox(
        FOOTER_TXT_LEFT, FOOTER_TXT_TOP, FOOTER_TXT_W, FOOTER_TXT_H
    )
...
def add_card(slide, left, top, width, height, header_text, body_text):
    ...
    tb = slide.shapes.add_textbox(
        left + padding, top + padding, width - 2 * padding, Emu(274320)
    )
    para = tb.text_frame.paragraphs[0]
    ...
    tb2 = slide.shapes.add_textbox(
        left + padding, top + Emu(320040), width - 2 * padding, height - Emu(365760)
    )
...
def update_tech_pitch():
    ...
    steps = [
        (
            "1  DECOMPOSE",
            "PlannerAgent breaks idea into parallel tasks (9 types: BACKEND, FRONTEND, TESTS, etc.)",
        ),
        (
            "2  CRITIC REVIEW",
            "Actor-critic loop: Builder\u2194Critic iterates until score \u2265 threshold (default 70/100)",
        ),
        (
            "3  EXECUTE",
            "Wave-based parallel execution via ReAct pattern (Thought\u2192Action\u2192Observation\u2192Status)",
        ),
        (
            "4  INTEGRATE",
            "Merge results, run tests, generate diff \u2014 critic reviews integration",
        ),
        (
            "5  APPROVE",
            "Human sees artifact + critic report + revision history \u2192 approve or reject",
        ),
    ]
    ...
    add_bottom_note(
        slide_a,
        "3-tier degradation: External Open SWE \u2192 LangGraph workflow \u2192 ReAct LLM fallback",
        top=Emu(4480560),
    )
    add_footer(slide_a)
    ...
    add_title(
        slide_b,
        "Agentic Patterns for Built Products",
    ...
    for i, pattern in enumerate(left_patterns):
        ...
        rect = slide_b.shapes.add_shape(
            MSO_SHAPE.RECTANGLE, col1_left, top, col_w, row_h
        )
        rect.fill.solid()
        rect.fill.fore_color.rgb = CARD_BG
        rect.line.fill.background()
        tb = slide_b.shapes.add_textbox(
            col1_left + Emu(137160),
            top + Emu(109728),
            col_w - Emu(274320),
            row_h - Emu(137160),
        )
    ...
    for i, pattern in enumerate(right_patterns):
        top = start_top + i * (row_h + Emu(91440))
        rect = slide_b.shapes.add_shape(
            MSO_SHAPE.RECTANGLE, col2_left, top, col_w, row_h
        )
        rect.fill.solid()
        rect.fill.fore_color.rgb = CARD_BG
        rect.line.fill.background()
        tb = slide_b.shapes.add_textbox(
            col2_left + Emu(137160),
            top + Emu(109728),
            col_w - Emu(274320),
            row_h - Emu(137160),
        )
    ...
    add_bottom_note(
        slide_b,
        "3 HITL levels: minimal (2 gates) \u00b7 standard (3 gates) \u00b7 strict (every stage)",
...
def update_business_case():
    ...
    add_title(
        slide,
        "Build Orchestrator \u2014 Product Factory",
    ...
    props = [
        (
            "10x Faster",
            "Parallel agent execution with dependency-aware wave scheduling",
        ),
        (
            "Quality Built In",
            "Critic agent reviews every artifact before human sees it (actor-critic pattern)",
        ),
        (
            "Human Always in Control",
            "3 configurable HITL levels: minimal, standard, strict \u2014 compliance guaranteed",
        ),
    ]
    ...
    add_bottom_note(
        slide,
        "Supports 10 agentic AI patterns (Google Cloud reference) \u00b7 9 task types \u00b7 3-tier degradation",
        top=Emu(4937760),
    )
    add_footer(slide)
...
def update_investor_pitch():
    ...
    add_title(
        slide,
        "Build Orchestrator \u2014 The Meta-Agent Factory",
    ...
    diffs = [
        (
            ...
            "Every output stress-tested before humans see it. Score improves across iterations.",
        ),
        (
            "10 Agentic Architectures",
            "Built products can use any Google Cloud reference pattern \u2014 not just chat bots.",
        ),
        (
            "Compounding Intelligence",
            "Critic feedback stored in vector memory \u2014 quality improves with every build.",
        ),
    ]
    ...
    add_bottom_note(
        slide,
        "One human + SAGE = billion-dollar company infrastructure",
        top=Emu(5166360),
    )
    add_footer(slide)
...
if __name__ == "__main__":
    ...
    print("\n=== Verification ===")
    for path in [
        "docs/SageAI_Tech_Pitch.pptx",
        "docs/SageAI_Business_Case.pptx",
        "docs/SageAI_Investor_Pitch.pptx",
    ]:
        prs = Presentation(path)
        ...
        for i, slide in enumerate(prs.slides):
            ...
            marker = (
                " <<<NEW"
                if "Build Orchestrator" in title or "Agentic Patterns" in title
                else ""
            )
