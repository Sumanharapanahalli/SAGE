...
The HITL approval gate is not bureaucracy — it is the product. SAGE exists to amplify human judgment, never to replace it. Compliance guarantees in regulated industries depend on this. Any solution-level agent proposal (`yaml_edit`, `knowledge_delete`, `agent_hire`) must have a human in the loop before execution. Framework control operations (`config_switch`, `llm_switch`, `config_modules`) are intentionally immediate — they are tool operation, not agent action. The distinction matters: agent proposals carry compliance risk; framework control is the operator's own action.

**1a. For CODE, the gate is the Merge Request — not the individual change.** (Merge-Gate Governance.) Agents autonomously write code, run the evidence gate, and iterate on an **isolated branch**; nothing per-change is human-gated there. The human's single control point is the **MR**: a regulatory-complete pull request (high-level summary, system impact, risk assessment, traceability, green test/build evidence, updated docs) that the human approves or returns with high-level comments. Only an **approved MR merges to `main`**, and the merge writes a **signed, tamper-evident audit record** (HMAC hash-chain). This is deliberately how regulated software is actually released — a reviewed, signed, traceable merge to a protected branch — and it makes human time the scarce resource, spent only at the top. Because the branch is the workspace and the MR is the gate, `code_diff`/`implementation_plan` are internal steps on the branch, not standalone human gates.
