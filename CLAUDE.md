...
### 🖥️ Interfaces
- **[Desktop (sage-desktop)](.claude/docs/interfaces/desktop-gui.md)** — No-sockets Tauri + Python sidecar app; when to use vs. web/VS Code. Phase 2 ships Settings + Backlog pages and a queue tile on Status. Phase 3a adds the runtime solution switcher (sidecar respawn + `solution-switched` event). Phase 3c adds the onboarding wizard (`/onboarding` route + `onboarding.generate` RPC) so new solutions can be created without FastAPI. Phase 3d adds the Build Console (`/builds` route + `builds.{start,list,get,approve}` RPC) so the BuildOrchestrator can be driven end-to-end from the desktop. Phase 3b adds the `/yaml` route + `yaml.{read,write}` RPC so the solution's YAML triad is editable without leaving the app (YAML syntax validated sidecar-side before the file is touched). Phase 5a adds the `/evolution` route + `evolution.{leaderboard,history,analytics,train}` RPCs so Agent Gym's learning loop is usable without FastAPI.
