# Task 2: Structured error events in agents
...
Replace bare `except Exception as e: logger.error(str(e))` blocks in src/agents/analyst.py, developer.py, and universal.py with structured error events: emit a dict with keys `{event:'agent_error', agent:..., task_id:..., error_type:..., message:..., trace_id:...}` to the audit_logger on failure. Keep the same external behaviour.

## Criteria

All bare except blocks are replaced; audit_logger receives structured dicts on agent failure; no new bare excepts introduced; existing tests pass.
